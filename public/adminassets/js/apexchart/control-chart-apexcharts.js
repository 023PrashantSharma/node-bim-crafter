...
function chartL() {
    var options;
    $.ajax({ 
        url: `${admin}/get-total-add-money-or-withdraw-money`,
        type: 'POST',
        data: {},
        dataType: "json",
        success: function (data)
        {	
            if(data.total_added == 0 && data.total_withdraw == 0){
                $("#err_smg").html('No Transaction Found');
            }else{
                if(data.status == "success"){
                    options = {
                        ...
                        labels: ['Added Money', 'Withdraw Money'],
                        series: [data.total_added, data.total_withdraw],
                        responsive: [{
                            breakpoint: 480,
                            options: {
                                chart: {
...
/* Pie Chart */
function getMonthWiseChartL(val) {
    if(val == ""){
        chartL();
    }else{
        var options;
        $.ajax({ 
            url: `${admin}/get-monthwise-total-add-money-or-withdraw-money`,
            type: 'POST',
            data: {},
            dataType: "json",
            success: function (data)
